...
int main(int argc, char *argv[])
{
	...
	cout << "# of threads: " << _numThreads << " / " <<  omp_get_num_procs() << endl;
	cout << endl;
  ...
  // parallel clause
  #pragma omp parallel num_threads(_numThreads) default(none) shared(wm, cells, cout, _numThreads) 
  //wm, cells and cout _numThreads is defined outside the parallel region
  {
	// set the number of threads for the current thread
	...
	for (int r = 0; r < wm.num_rows(); r++) {
		for (int c = 0; c < wm.num_cols(); c++) {
			...
			//
			#pragma omp atomic	// increment the cell count atomically
			cells++;
			
			#pragma omp critical	// only on tread can print at a time
			{
				if (cells % 100 == 0) {
					cout << ".";
					cout.flush();
